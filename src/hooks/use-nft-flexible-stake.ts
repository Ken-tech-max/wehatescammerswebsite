...
import { STAKE_DATA_FLEXIBLE_SIZE, STAKE_CONTRACT_FLEXIBLE_IDL } from '../utils/constant';

const { metadata: { Metadata } } = programs;
const TOKEN_METADATA_PROGRAM_ID = new anchor.web3.PublicKey("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s");
const rpcHost = process.env.NEXT_PUBLIC_SOLANA_RPC_HOST!;
const connection = new anchor.web3.Connection(rpcHost);
const programId = new PublicKey(process.env.NEXT_PUBLIC_STAKE_FLEXIBLE_CONTRACT_ID!);
const pool = new PublicKey(process.env.NEXT_PUBLIC_STAKE_FLEXIBLE_POOL_ID!);
const idl = STAKE_CONTRACT_FLEXIBLE_IDL as anchor.Idl;
...
const getStakedNftsForOwner = async (wallet: AnchorWallet) => {
    ...
    try {
        let resp = await connection.getProgramAccounts(programId,
            {
                ...
                filters: [
                    {
                        dataSize: STAKE_DATA_FLEXIBLE_SIZE
                    },
                    {
                        memcmp:{
                            offset: 9,
        ...
        for(let nftAccount of resp){
            ...
            allTokens.push({
                withdrawnNumber : stakedNft.withdrawnNumber.toNumber(),
...
export default useNftFlexibleStake;